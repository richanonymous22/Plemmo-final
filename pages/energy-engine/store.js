/* ════════════════════════════════════════════════════════════════════════
   PLEMMO · BUSINESS ENERGY — browser-local store
   ────────────────────────────────────────────────────────────────────────
...
(function (global) {
  ...
  var K_CONFIG = 'plemmo_energy_config_v1';
  var K_LEADS = 'plemmo_energy_leads_v1';
  var K_AUDIT = 'plemmo_energy_audit_v1';
  var K_PASS = 'plemmo_energy_admin_pass_v1';
  var K_UNLOCK = 'plemmo_energy_admin_unlocked';
  ...
  function getConfig() { var stored = readJSON(K_CONFIG, null); return stored || clone(global.PLEMMO_ENERGY_CONFIG); }
  ...
  function importConfigJSON(jsonString) {
    ...
    if (!parsed || !parsed.solutionTypes || !parsed.businessTypes) throw new Error('That file does not look like a Business Energy config export.');
  ...
  function leadsToCSV() {
    var leads = getLeads();
    var cols = ['id', 'createdAt', 'status', 'businessName', 'contactName', 'telephone', 'email', 'businessTypeId', 'energyRequiredId', 'currentSupplierId', 'contractStatusId', 'annualSpendBandId', 'locationCountBandId', 'recommendedSolution', 'hasAttachment', 'notes'];
    var rows = [cols.join(',')];
    leads.forEach(function (l) {
      rows.push(cols.map(function (c) { var v = l[c]; v = (v === undefined || v === null) ? '' : String(v); return '"' + v.replace(/"/g, '""') + '"'; }).join(','));
    });
    return rows.join('\r\n');
  }
