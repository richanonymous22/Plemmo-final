/* ════════════════════════════════════════════════════════════════════════
   PLEMMO · DIGITAL SIGNAGE & MENU DESIGN — browser-local store
   ────────────────────────────────────────────────────────────────────────
...
(function (global) {
  ...
  var K_CONFIG = 'plemmo_signage_config_v1';
  var K_LEADS = 'plemmo_signage_leads_v1';
  var K_AUDIT = 'plemmo_signage_audit_v1';
  var K_PASS = 'plemmo_signage_admin_pass_v1';
  var K_UNLOCK = 'plemmo_signage_admin_unlocked';
  ...
  function getConfig() { var stored = readJSON(K_CONFIG, null); return stored || clone(global.PLEMMO_SIGNAGE_CONFIG); }
  ...
  function importConfigJSON(jsonString) {
    ...
    if (!parsed || !parsed.screenSizes || !parsed.businessTypes) throw new Error('That file does not look like a Digital Signage config export.');
  ...
  function leadsToCSV() {
    var leads = getLeads();
    var cols = ['id', 'createdAt', 'status', 'businessName', 'contactName', 'mobile', 'email', 'businessTypeId', 'serviceIds', 'screenSizeId', 'numberOfScreens', 'notes'];
    var rows = [cols.join(',')];
