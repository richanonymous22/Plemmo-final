/* ════════════════════════════════════════════════════════════════════════
   PLEMMO · EPOS RECOMMENDATION ENGINE — browser-local store
   ────────────────────────────────────────────────────────────────────────
   Independent copy of the same browser-local bridge pattern used by the
   Card Machine Recommendation Engine (Phase 1) — separate localStorage
   keys, separate file, no shared code, so the two modules can evolve
   (and eventually move to a real backend) independently.

   This site has no backend/database. The admin panel edits a working
   copy of config.js persisted here; leads and the audit log live here
   too. Not real multi-user infrastructure — one browser/device only.
   ════════════════════════════════════════════════════════════════════════ */
(function (global) {
  'use strict';

  var K_CONFIG = 'plemmo_epos_config_v1';
  var K_LEADS = 'plemmo_epos_leads_v1';
  var K_AUDIT = 'plemmo_epos_audit_v1';
  var K_PASS = 'plemmo_epos_admin_pass_v1';
  var K_UNLOCK = 'plemmo_epos_admin_unlocked';
  ...
  function getConfig() {
    ...
    return stored || clone(global.PLEMMO_EPOS_CONFIG);
  }
  ...
  function importConfigJSON(jsonString) {
    ...
    if (!parsed || !parsed.packages || !parsed.categories) {
      throw new Error('That file does not look like an EPOS Recommendation Engine config export.');
    }
  ...
  function updateLead(id, patch) {
    ...
    for (var i = 0; i < leads.length; i++) {
      if (leads[i].id === id) { leads[i] = Object.assign({}, leads[i], patch); writeJSON(K_LEADS, leads); appendAudit('lead', 'Lead updated: ' + id); return leads[i]; }
    }
    return null;
  }
  function deleteLead(id) {
    writeJSON(K_LEADS, getLeads().filter(function (l) { return l.id !== id; }));
    appendAudit('lead', 'Lead deleted: ' + id);
  }
  function leadsToCSV() {
    var leads = getLeads();
    var cols = ['id', 'createdAt', 'status', 'businessName', 'contactName', 'mobile', 'email', 'categoryId', 'typeId', 'recommendedPackageId', 'needsReview', 'notes'];
    var rows = [cols.join(',')];
    leads.forEach(function (l) {
      rows.push(cols.map(function (c) {
        var v = l[c]; v = (v === undefined || v === null) ? '' : String(v);
        return '"' + v.replace(/"/g, '""') + '"';
      }).join(','));
    });
    return rows.join('\r\n');
